...
### 准备工作
1. 改造路由链接
2. 新建PhotoList.vue 组件，添加对应的路由规则
### 绘制图片列表组件 页面结构并美化样式
1. 制作 顶部的滑动条  
  + 参考 MUI tab-top-webview-main.html  
  + 需要把slider区域的mui-fullscreen 类去掉
  + 滑动条无法正常触发滑动，通过检查官方文档，发现这是一个JS组件，需要被初始化
    - 导入mui.js
    - 调用官方提供的方法去初始化：
    ```
    ...
    ```
  + 初始化滑动条导入的mui.js引起报错，mui.js使用了'caller' 'claaee' 'arguments',但是，webpack打包好的bundle.js中默认使用严格模式，两者发生冲突
    - 解决方法：要么改变 mui.js 的非严格模式，但是不现实；要么禁用webpack打包的严格模式，使用插件  babel-plugin-transform-remove-strict-mode
  + 解决tabbar不能点击切换问题
  + 渲染分类列表的数据

# 2020.3.29 
2. 制作 底部的图片列表
  1. 借助Mint-UI提供的现成组件`lazy-load`来实现图片列表的懒加载技术
...
2. 新增 图片详情 组件 和 路由规则
3. 图片详情组件的结构和样式
4. 每次进入图片详情初始化渲染数据，并且更新点击数
5. > 图片缩略图的制作
   1. 使用   [`vue-preview`](https://github.com/LS1231/vue-preview)，一个Vue集成PhotoSwipe图片预览插件
